...
javascript 的类型主要分为 6 种基础类型和对象类型。
原始类型包括：null、undefined、boolean、number、string 和 es6 中的 symbol。

- 布尔：就是简单的 true 和 false，该类型叫做`boolean`，在 typescript 中定义布尔类型如下：
...
- 空值 void：只能赋值 null 和 undefined。声明一个 void 函数时，表示该函数没有任何返回值，声明一个 void 变量时，并没什么实际用途，该变量只能赋值 null 或 undefined。
...
- 类型推论：如果没有明确指出一个类型，typescript 会推断出一个类型。如果定义时没有赋值，不管之后有没有赋值，都会被推断成 any。
...
- 联合类型：表示取值可以是多种类型中的一种，使用 | 分割每个类型。

* 访问联合类型：当 typescript 不能确定访问的是联合类型中的哪一个的时候，我们只能访问联合类型中的共有的属性或方法。
